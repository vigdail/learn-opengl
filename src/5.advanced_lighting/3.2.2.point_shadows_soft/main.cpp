...
int main()
{
    ...
    Shader depthMapShader("depthmap.vs", "depthmap.fs", "depthmap.gs");
    ...
    glm::vec3 cubePositions[3] = {
        ...
        glm::vec3(-4.25f, 2.0f, -4.5f),
        glm::vec3(2.2f, 0.0f, 1.0f),
    };

    glm::vec3 movingCubePosition = glm::vec3(2.0f, 1.5f, -1.0f);
    ...
    glm::vec3 lightPosition = glm::vec3(0.0f, 2.5f, 0.0f);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        {
            depthMapShader.setMatrix4("shadowMatrices[" + std::to_string(i) + "]", shadowTransforms[i]);
        }
        ...
        model = glm::mat4(1.0);
        model = glm::translate(model, movingCubePosition);
        float offsetX = 5.0f * (float)sin(glfwGetTime());
        float offsetZ = 5.0f * (float)cos(glfwGetTime());
        ...
        shader.setFloat("lightFarPlane", far);
