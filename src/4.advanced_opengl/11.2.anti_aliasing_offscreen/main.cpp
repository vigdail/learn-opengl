...
int main()
{
    ...
    glRenderbufferStorageMultisample(GL_RENDERBUFFER, samples, GL_DEPTH24_STENCIL8, WIN_WIDTH, WIN_HEIGHT);

    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, rbo);

    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
    {
        std::cerr << "Multisampled Framebuffer is incomplete" << std::endl;
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // from multisampled to intermediate frame buffer
    ...
    glDeleteFramebuffers(1, &fboMultisampled);
