...
namespace core
{
    ...
    Camera::Camera() : position(glm::vec3(0.0f, 0.0f, 0.0f)), front(glm::vec3(0.0f, 0.0f, -1.0f)), worldUp(glm::vec3(0.0f, 1.0f, 0.0f)),
                       yaw(YAW), pitch(PITCH), speed(SPEED), sensitivity(SENSITIVITY), fov(FOV)
    {
        updateVectors();
    }

    Camera::Camera(glm::vec3 position, glm::vec3 up, float yaw, float pitch) : position(position), front(glm::vec3(0.0f, 0.0f, -1.0f)), worldUp(up),
                                                                               yaw(yaw), pitch(pitch), speed(SPEED), sensitivity(SENSITIVITY), fov(FOV)
    {
        updateVectors();
    }
